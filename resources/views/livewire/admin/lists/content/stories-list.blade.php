<div>
    ...
    <form class="filter-section mb-3 px-3 py-2 border position-relative bg-light {{ $filter ? '' : 'd-none' }}"
        ...
        <div class="row table-filters-row">
            ...
            <div class="form-group col-md-3">
                <label class="form-label fw-500 mb-1">Story Title</label>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr class="position-relative">
                    <td>
                        <input class="form-check-input" type="checkbox" wire:model="selectedStories"
                            value="{{ $story->id }}">
                    </td>
                    <td>
                        <a href="/admin/content/stories/{{ $story->id }}/edit">
                    ...
                    <td>{{ Str::limit($story->short_description, 60) }}</td>
                    <td>{{ $story->is_featured ? 'Yes' : 'No'}}</td>
                    <td>{{ $story->is_active ? 'Yes' : 'No'}}</td>
                ...
                @empty
                <tr>
                    <td colspan="6" class="text-center py-3">No stories found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <nav aria-label="..." class="pagination mt-3 mb-2 d-flex justify-content-end align-items-center">
        ...
        <ul class="pagination pb-0 mb-0">
            <!-- Previous -->
            <li class="page-item {{ $stories->onFirstPage() ? 'disabled' : '' }}">
                <a href="#" class="page-link text-dark"
                    wire:click.prevent="goToPage({{ $stories->currentPage() - 1 }})">Previous</a>
            </li>

            <!-- Page Numbers -->
            @foreach(range(1, $stories->lastPage()) as $p)
            <li class="page-item {{ $stories->currentPage() == $p ? 'active' : '' }}">
                <a href="#"
                    class="page-link {{ $stories->currentPage() == $p ? 'bg-dark text-white border-dark' : 'text-dark' }}"
            ...
            <!-- Next -->
            <li
                class="page-item {{ $stories->currentPage() == $stories->lastPage() ? 'disabled' : '' }}">
                <a href="#" class="page-link text-dark"
                    wire:click.prevent="goToPage({{ $stories->currentPage() + 1 }})">Next</a>
            </li>
        </ul>
    </nav>
