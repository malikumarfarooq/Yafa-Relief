    ...
    <form class="filter-section mb-3 px-3 py-2 border position-relative bg-light {{ $filter ? '' : 'd-none' }}"
        ...
        <div class="row table-filters-row">
            ...
            <div class="form-group col-md-3">
                <label class="form-label fw-500 mb-1">Post Title</label>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr class="position-relative">
                    <td>
                        <input class="form-check-input" type="checkbox" wire:model="selectedPosts"
                            value="{{ $post->id }}">
                    </td>
                    <td>
                    ...
                    <td>{{ Str::limit($post->short_description, 60) }}</td>
        ...
        <ul class="pagination pb-0 mb-0">
            ...
            <li class="page-item {{ $posts->onFirstPage() ? 'disabled' : '' }}">
                <a href="#" class="page-link text-dark"
                    wire:click.prevent="goToPage({{ $posts->currentPage() - 1 }})">Previous</a>
            </li>

            <!-- Page Numbers -->
            @foreach(range(1, $posts->lastPage()) as $p)
            <li class="page-item {{ $posts->currentPage() == $p ? 'active' : '' }}">
                <a href="#"
                    class="page-link {{ $posts->currentPage() == $p ? 'bg-dark text-white border-dark' : 'text-dark' }}"
            ...
            <li
                ...
                <a href="#" class="page-link text-dark"
                    wire:click.prevent="goToPage({{ $posts->currentPage() + 1 }})">Next</a>
            </li>
        </ul>
    </nav>
