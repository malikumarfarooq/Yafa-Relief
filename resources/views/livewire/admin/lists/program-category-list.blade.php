<div>
    ...
    <form class="filter-section mb-3 px-3 py-2 border position-relative bg-light {{ $filter ? '' : 'd-none' }}"
        ...
        <div class="row table-filters-row">
            ...
            <div class="form-group col-md-3">
                <label class="form-label fw-500 mb-1">Category Name</label>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th class="highlight-text">Category Name</th>
            ...
            <tbody>
                ...
                <tr class="position-relative">
                    <td>
                        <input class="form-check-input" type="checkbox" wire:model="selectedProgramCategories"
                            value="{{ $programCategory->id }}">
                    </td>
                    <td>
                        <a href="/admin/programs/categories/{{ $programCategory->id }}/edit">
                            <img src="{{ $programCategory->avatar
                                    ? asset('storage/'.$programCategory->avatar)
                    ...
                    <td>{{ Str::limit($programCategory->description, 60) }}</td>
                ...
                @empty
                <tr>
                    <td colspan="6" class="text-center py-3">No programs category found.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    <nav aria-label="..." class="pagination mt-3 mb-2 d-flex justify-content-end align-items-center">
        ...
        <ul class="pagination pb-0 mb-0">
            <!-- Previous -->
            <li class="page-item {{ $programCategories->onFirstPage() ? 'disabled' : '' }}">
                <a href="#" class="page-link text-dark"
                    wire:click.prevent="goToPage({{ $programCategories->currentPage() - 1 }})">Previous</a>
            </li>

            <!-- Page Numbers -->
            @foreach(range(1, $programCategories->lastPage()) as $p)
            <li class="page-item {{ $programCategories->currentPage() == $p ? 'active' : '' }}">
                <a href="#"
                    class="page-link {{ $programCategories->currentPage() == $p ? 'bg-dark text-white border-dark' : 'text-dark' }}"
            ...
            <!-- Next -->
            <li
                class="page-item {{ $programCategories->currentPage() == $programCategories->lastPage() ? 'disabled' : '' }}">
                <a href="#" class="page-link text-dark"
                    wire:click.prevent="goToPage({{ $programCategories->currentPage() + 1 }})">Next</a>
            </li>
        </ul>
    </nav>
